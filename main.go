...
func MapFileHandler(w http.ResponseWriter, r *http.Request) {
	...
	if strings.Contains(contentType, "image/") && strings.Contains(reqPath, ".png") {
		tileMap, err := tiled.LoadFromFile(
			strings.Replace(filePath, ".png", ".tmx", 1),
		)
...
func main() {
	...
	r.HandleFunc("/maps/{mapfile}", MapFileHandler)
